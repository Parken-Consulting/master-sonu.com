...
function cal_navigate(month,year,evt) {
	setFade(0);
	var url = "/includes/calendar/super_calendar.php?month="+month+"&year="+year+"&event="+evt;
...
function showJump(obj) {
	...
	output += '</select> <a href="javascript:jumpTo()"><img src="/includes/calendar/images/calGo.gif" alt="go" /></a> <a href="javascript:hideJump()"><img src="/includes/calendar/images/calStop.gif" alt="close" /></a>';
...
function jumpTo() {
	...
	cal_navigate(m.options[m.selectedIndex].value,y.options[y.selectedIndex].value,'');
	hideJump();
}